...
"""
Amended by Sam Richards for evaluation and timing.
Coursework: LWC for IoT Devices
14 May 2023
"""
...
#    print("=== demo encryption using {variant} ===".format(variant=variant))    #SamR--print functions turned off
    ...
    associateddata = AD    #SamR--changed to the AD variable 
    plaintext      = PT    #SamR--changed to the PT variabl

    ciphertext        = ascon_encrypt(key, nonce, associateddata, plaintext,  variant)
#    receivedplaintext = ascon_decrypt(key, nonce, associateddata, ciphertext, variant)    #SamR--decrypt function turned off

#    if receivedplaintext == None: print("verification failed!")

    
#    demo_print([("key", key),                   #SamR--print functions turned off
#                ("nonce", nonce), 
#                ("plaintext", plaintext), 
...
if __name__ == "__main__":                        #SamR--demo functions removed

    PT=str.encode("5345,766,54,67,4578,98,43,2")  #SamR--27 bytes
    AD=str.encode("1234")                         #SamR--4 bytes

    import timeit                                 #SamR--timeit module 
    setup_code_1="""                              
aead="Ascon-128"
hash="Ascon-Hash"
"""                                               #SamR--set up code that is excluded from timing



print (timeit.repeat (stmt="demo_aead(aead)",     #SamR--function to time
                    setup=setup_code_1,
                      globals=globals(),
                      number=1000,
                      repeat=10))
